	...
	public void configure() throws Exception {
		...
		rest().tag("API de serviços Demo utilizando Camel e Quarkus").produces("application/json")				
			...
			.get("/clientes")				
				...
				.route().routeId("restclienteall")
				.to("direct:clienteall")
				.choice()
				.when(body().isNull())
			...
			.get("/clientes/{cpf}")
				...
				.route().routeId("restquerybycpf")
				.to("direct:querybycpf").choice()
				.when(body().isNull())
				.setHeader(Exchange.HTTP_RESPONSE_CODE, constant(404))
				.endChoice().otherwise()
				.setHeader(Exchange.HTTP_RESPONSE_CODE, constant(200))
			...
			.post("/clientes/{cpf}/{nome}/{email}/{endereco}")
				...
				.route().routeId("restclienteinsert") 
				.to("direct:clienteinsert")
		...
		from("direct:clienteall").routeId("clienteall")
		.to("sql:classpath:sql/queryall.sql");

		from("direct:querybycpf").routeId("querybycpf")
		.to("sql:classpath:sql/querybycpf.sql?outputType=SelectOne&outputClass=com.redhat.camel.model.Cliente");
		...
		from("file:{{pedidos.in.folder}}?antInclude=**/*.xml&move={{pedidos.out.folder}}").routeId("pedidos")
			.choice()
				.when(xpath("/pedido/departamento='TI'"))
				.to(ExchangePattern.InOnly,"seda:processarPedido")
				.otherwise()
				.log(LoggingLevel.WARN, "Pedido ignorado ${header.CamelFileName}");
		...
		from("direct:criarNovoPedido").routeId("criarNovoPedido")			
			...
			.setHeader(Exchange.CONTENT_TYPE,constant("application/json"))
			.marshal().json()			
			.to("http://{{novo.sistema.pedido.url}}:8085/pedidos").log("Pedido processado [${header.CamelHttpResponseCode}-${header.CamelHttpResponseText}]")
		;
	}
